...
Then /^I (should|should not) see ckey "([^\"]*)" in the folder$/ do |comparator, arg1|
  case comparator
    when "should"
      page.should have_selector("tr", :id => "#{arg1}")
    when "should not"
      page.should_not have_selector("tr", :id => "#{arg1}")
    end
end
...
Then /^I (should|should not) see the Starred Items tools$/ do |comparator|
  case comparator
    when "should"
      page.should have_selector("ul#markedListTools")
    when "should not"
      page.should_not have_selector("ul#markedListTools")
    end
end 